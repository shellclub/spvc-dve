...
import { Button, Select, Spinner, Card, Label } from "flowbite-react"
import {
  IconChevronLeft,
  IconChevronRight,
  IconChevronsLeft,
...
const SupervisionViewTable = () => {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginationTableType[]>(
    !selected
      ? "/api/students/withSupervisions/all" // ◀◀◀ API ใหม่
      : `/api/students/withSupervisions/all?term=${selectedTerm}&year=${selectedYear}`,
    fetcher,
  )
  ...
  const filteredStudents = useMemo(() => {
    ...
    return stdData.filter((user) => {
      ...
      return matchesMajor && matchesGrade && matchesRoom && matchesDepartment // ◀◀◀ แก้ไข: คืนค่าให้ครบ
    })
  }, [stdData, majorFilter, gradeFilter, roomFilter, departmentFilter])
      ...
      <TitleIconCard title="ข้อมูลการนิเทศนักศึกษา">
        ...
        <div className="flex flex-wrap gap-4 mb-4">
          <div className="flex-1 min-w-[200px]">
            ...
            >
              <option hidden value="all">แผนกวิชา</option>
              <option value="all">ทั้งหมด</option>
              {departments.map((dept, index) => (
                <option key={index} value={dept.depname}>
                  {dept.depname}
                </option>
              ))}
            </Select>
