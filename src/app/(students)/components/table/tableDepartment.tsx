...
interface TableDepartmentProps {
  data: PaginationTableType[];
  onEdit: (depname: string, id: string) => void;
}

const columnHelper = createColumnHelper<PaginationTableType>();

const TableDepartment: React.FC<TableDepartmentProps> = ({ data,onEdit }) => {
  ...
  const columns = [
    ...
    columnHelper.display({
      ...
      cell: (info) => (
        ...
        >
          {[
            { icon: "tabler:edit", listtitle: "Edit", onclick: () => onEdit(info.row.original.depname as string, info.row.original.id as string) },
