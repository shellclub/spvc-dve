...
export async function POST(request: NextRequest) {
    ...
    if (!company) {
        return NextResponse.json(
            { message: "ไม่พบข้อมูลการฝึกงานของนักศึกษา ติดต่อครูนิเทศ", type: "error" },
            { status: 404 }
        );
    }

    const existingInternship = await prisma.inturnship.findUnique({
        where: {
            studentId: Number(user.student.id)
        }
    });
    ...
    if (existingInternship) {
        return NextResponse.json(
            { message: "นักศึกษานี้มีข้อมูลการฝึกงานอยู่แล้ว", type: "error" },
            { status: 400 }
        );
    }
